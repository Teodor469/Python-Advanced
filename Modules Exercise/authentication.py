...
def get_users_data():
    ...
    with open("Modules Exercise/db.txt", "r") as users_file:
...
def registration():
    ...
    if check_registration(info_dict):
        with open("Modules Exercise/db.txt", "a") as users_file:
...
def check_registration(info_dict):
    ...
    for user in users_data:
        if "Username" in user and user["Username"] == info_dict["Username"]:
            frame.create_text(
                200,
                300,
...
def check_login():
    ...
    for user in users_data:
        ...
        if current_user_username is None or current_user_password is None:
            continue
