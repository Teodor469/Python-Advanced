...
while materials and magic_levels:
    # Fix: Use popleft() to get the first element from the deque
...
# Fix: Use reverse() to reverse the order of materials
if materials:
    print(f"Materials left: {', '.join(str(x) for x in materials[::-1])}")
# Fix: Use list comprehension to convert magic_levels to a list
if magic_levels:
    print(f"Magic left: {', '.join(str(x) for x in list(magic_levels))}")

# Fix: Use a list comprehension to print crafted presents
print('\n'.join([f"{toy}: {crafted.count(toy)}" for toy in sorted(set(crafted))]))
